...
class AmclNode(Node):
    ...
    def timer_callback(self):
        ...
        # CASO 2: Tiene una ruta y la esta siguiendo
            ...
            if abs(angle_error) > self.yaw_tolerance or self.check_for_imminent_obstacle():
        ...
        # CASO 3: Tiene un obstaculo inminente
            ...
            self.stop_robot()
            self.state = State.NAVIGATING
            # self.girar_hasta_cambiar(estimated_pose)
    ...
    def girar_hasta_cambiar(self, estimated_pose):
        self.get_logger().warn(f"Esta girando para evitar el obstaculo")
        robot_x = estimated_pose.position.x
        robot_y = estimated_pose.position.y
        _, _, robot_yaw = quaternion_to_euler(
                estimated_pose.orientation.x,
        ...
        if self.check_for_imminent_obstacle() or abs(angle_error) > self.yaw_tolerance:
            # angular_velocity = self.kp_ang * angle_error
            #     # Garantizar velocidad angular mínima para girar con suavidad
            ...
            twist_msg = Twist()
            twist_msg.linear.x = 0.0
            twist_msg.angular.z = self.obstacle_avoidance_turn_speed # Usar la velocidad de giro para evasión
            self.cmd_vel_pub.publish(twist_msg)
        else:
            self.get_logger().warn(f"Dejó de girar")
            self.stop_robot()

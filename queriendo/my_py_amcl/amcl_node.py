...
class AmclNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('obstacle_detection_distance', 0.1) # Distancia minima que considero que hay obstaculo inminente
        ...
        self.declare_parameter('safety_margin_cells', 3)  # Margen de seguridad en celdas
    ...
    def check_for_imminent_obstacle(self):
        ...
        front_angle_range = np.deg2rad(45)
    ...
    def timer_callback(self):
        ...
        # CASO 2: Tiene una ruta y la esta siguiendo
            ...
            # self.get_logger().warn(f"[DEBUG] Orientation_x: ({estimated_pose.orientation.x:.8f}")
            # self.get_logger().warn(f"[DEBUG] Orientation_y: ({estimated_pose.orientation.y:.8f}")
            # self.get_logger().warn(f"[DEBUG] Orientation_z: ({estimated_pose.orientation.z:.8f}")
            # self.get_logger().warn(f"[DEBUG] Orientation_w: ({estimated_pose.orientation.w:.8f}")
            ...
            if abs(angle_error) > self.yaw_tolerance:
                ...
                # FASE DE SEGUIMIENTO: Pure Pursuit cuando está alineado
        ...
        # CASO 3: Tiene un obstaculo inminente
            ...
            self.obstacle_avoidance_behavior(estimated_pose)
    ...
    def obstacle_avoidance_behavior(self, estimated_pose):
        """
        Comportamiento mejorado de evasión de obstáculos
        """
        ...
        if not self.check_for_imminent_obstacle():
            # Esperar un poco más antes de reanudar para asegurar que el camino esté completamente libre
            if not hasattr(self, 'obstacle_clear_count'):
                self.obstacle_clear_count = 0
            
            self.obstacle_clear_count += 1
            
            # Requerir varias verificaciones consecutivas de camino libre
            if self.obstacle_clear_count >= 5:  # 0.5 segundos con timer de 0.1s
                self.get_logger().warn('Camino despejado confirmado. Reanudando navegación')
                
                # Encontrar el punto más cercano en la ruta
                if self.current_path:
                    robot_pos = np.array([estimated_pose.position.x, estimated_pose.position.y])
                    dists = [np.linalg.norm(np.array(pt) - robot_pos) for pt in self.current_path]
                    closest_idx = int(np.argmin(dists))
                    self.current_path_index = min(closest_idx + 2, len(self.current_path) - 1)  # Avanzar un poco
                    self.get_logger().warn(f'Reanudando desde índice {self.current_path_index}')
                
                self.state = State.NAVIGATING
                self.obstacle_clear_count = 0
                return
            else:
                # Seguir girando mientras se verifica
                twist_msg = Twist()
                twist_msg.linear.x = 0.0
                twist_msg.angular.z = self.obstacle_avoidance_turn_speed
                self.cmd_vel_pub.publish(twist_msg)
                return
        else:
            # Resetear contador si se detecta obstáculo de nuevo
            self.obstacle_clear_count = 0
        
        # Calcular cuánto hemos girado
        if self.obstacle_avoidance_start_yaw is not None:
            yaw_diff = current_yaw - self.obstacle_avoidance_start_yaw
            yaw_diff = np.arctan2(np.sin(yaw_diff), np.cos(yaw_diff))  # Normalizar
            self.obstacle_avoidance_cumulative_angle = abs(yaw_diff)
        
        # Estrategia de evasión mejorada
        twist_msg = Twist()
        twist_msg.linear.x = 0.0  # No avanzar mientras hay obstáculo
        
        # Si hemos girado mucho (más de 180°), intentar la dirección opuesta
        if self.obstacle_avoidance_cumulative_angle > np.pi:
            self.get_logger().warn('Girando en dirección opuesta tras girar >180°')
            twist_msg.angular.z = -self.obstacle_avoidance_turn_speed
        else:
            # Girar en sentido antihorario (dirección positiva)
            twist_msg.angular.z = self.obstacle_avoidance_turn_speed
        
        self.cmd_vel_pub.publish(twist_msg)
        
        # Timeout de seguridad: si llevamos mucho tiempo evadiendo, replaneificar
        if not hasattr(self, 'obstacle_avoidance_timeout'):
            self.obstacle_avoidance_timeout = 0
        
        self.obstacle_avoidance_timeout += 1
        
        if self.obstacle_avoidance_timeout > 100:  # 10 segundos
            self.get_logger().error('Timeout en evasión de obstáculos. Replanificando ruta.')
            self.state = State.PLANNING
            self.obstacle_avoidance_timeout = 0
    ...
    def get_minimum_obstacle_distance(self):
        """
        Obtiene la distancia mínima a obstáculos en un cono frontal amplio
        """
        if self.latest_scan is None:
            return float('inf')
        
        min_distance = float('inf')
        front_angle_range = np.deg2rad(45)  # Cono amplio para seguridad
        ...
        return min_distance if min_distance != float('inf') else self.laser_max_range
